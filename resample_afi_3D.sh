...
# Usage: ./resample_afi_3D.sh <parent_dir> <input_fn> <reference_fn> <output_fn> <int_mode>
...
$fsl fslroi $reference_fn ref_tmp.nii.gz 0 -1 0 -1 0 -1 0 1    # cut the reference image to only two echoes
echo "Resampling input image to reference image space"
$ants antsApplyTransforms -d 3 -i $input_fn -r ref_tmp.nii.gz -o $output_fn -n $int_mode
