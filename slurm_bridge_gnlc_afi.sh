...
#   (i.e., GNLC is skipped), the script copies the AFI file to the GNLC output directory
#   with the correct BIDS-compliant name (inserting _desc-undistortedJac before _TB1AFI) and
#   submits a placeholder SLURM job so that downstream dependency chains remain intact.
#
# USAGE:
#   slurm_bridge_gnlc_afi.sh "<GNLC_COMMAND>" "<SUBJECT>" "<SESSION>" "<SOURCE_DIR>" "<OUTPUT_DIR>" "<GNLC_JOB_NAME>"
#
# ARGUMENTS:
...
if echo "$gnlc_output" | grep -q "already has NonlinearGradientCorrection=true"; then
    ...
    echo "Copying AFI files to GNLC output directory with correct naming..."
    echo "============================================================"
    ...
    if [[ -z "$filename" ]]; then
        ...
        if [[ ! -f "$src_file" ]]; then
            ...
            bids_suffix="_${basename_no_ext##*_}"  # e.g., _TB1AFI
    ...
    if [[ "$copy_success" == "true" ]]; then
        ...
        if [[ -n "$GNLC_JOB_NAME" ]]; then
            ...
            sbatch -p short,group_servers,gr_weiskopf \
                ...
                --time=5 --mem=100M \
                --output=/tmp/%j_placeholder_gnlc_afi.out \
                --wrap="echo 'GNLC AFI skipped - file already corrected. Placeholder job for dependency chain.'; sleep 180"
        fi
    else
        echo ""
        echo "  ============================================================"
        echo "  ERROR: AFI data is already corrected for gradient nonlinearities,"
